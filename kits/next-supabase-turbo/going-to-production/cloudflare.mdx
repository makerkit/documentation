...
### 3. Installing the Cloudflare CLI (optional!)
...
This allows you to build and run a preview of your site on Pages without deploying it.

Add the following scripts to the `apps/web/package.json` file:

```
"cloudflare:build": "pnpm dlx @cloudflare/next-on-pages",
"cloudflare:preview": "pnpm dlx wrangler pages dev .next --compatibility-flag=nodejs_compat",
```

To build a preview, run:

```bash
pnpm run cloudflare:build
```

To run the preview, run:
...
### 5. Select "apps/web" as path to build
...
### 6. Setting Node.js Compatibility Flags
...
1. Downgrading a version of Supabase Auth
2. Monkey-patching the `fetch` function for Keystatic

#### Downgrading Supabase Auth
