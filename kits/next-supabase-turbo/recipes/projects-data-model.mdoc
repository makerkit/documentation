...
Now that we have a data model, let's create the UI for managing projects. We will create a new page at `apps/web/app/home/[account]/projects/page.tsx` that will display a list of projects for the current account.

### Adding Translations

Before we add the navigation link, we need to add the translation for the "Projects" route. Add the following to your translation files:

In `apps/web/public/locales/en/common.json`, add to the `routes` section:
...
Let's update the navigation menu to add a new link to the projects page. Update the team account navigation configuration at `apps/web/config/team-account-navigation.config.tsx` by adding:
...
We can now create the page at `apps/web/app/home/[account]/projects/page.tsx` with the following content:
...
### Creating the Schema for Project Creation

First, we need to create a schema for the project creation form. Create a new file at `apps/web/app/home/[account]/projects/_lib/schema/create-project-schema.ts`:

```typescript {% title="apps/web/app/home/[account]/projects/_lib/schema/create-project-schema.ts" %}
import { z } from 'zod';
...
We can now create the dialog to create a project. We will create a new file at `apps/web/app/home/[account]/projects/_components/create-project-dialog.tsx` with the following content:

 ```tsx {% title="apps/web/app/home/[account]/projects/_components/create-project-dialog.tsx" %}
'use client';
...
We can now create the server action to create a project. We will create a new file at `apps/web/app/home/[account]/projects/_lib/server/server-actions.ts` with the following content:

 ```tsx {% title="apps/web/app/home/[account]/projects/_lib/server/server-actions.ts" %}
'use server';
...
We will create a new file at `apps/web/app/home/[account]/projects/[id]/layout.tsx` with the following content:
