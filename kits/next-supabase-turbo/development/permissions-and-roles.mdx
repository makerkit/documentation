...
- `roles_permissions` table: This table stores the permissions for each role.

The table `roles_permissions` has the following schema:
...
You can assign these permissions to roles in the `roles_permissions` table for fine-grained access control:

```sql
insert into public.roles_permissions (role, permission) values ('owner', 'tasks.write');
insert into public.roles_permissions (role, permission) values ('owner', 'tasks.delete');
```
