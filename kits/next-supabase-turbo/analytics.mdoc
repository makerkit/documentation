...
[Core Concepts of the Analytics API](#core-concepts-of-the-analytics-api)

[Using the Analytics API](#using-the-analytics-api)

[Integration with App Events](#integration-with-app-events)

[Extending the Analytics API](#extending-the-analytics-api)
...
The Analytics API is available through the `analytics` object imported from `@kit/analytics`, which runs exclusively on the client side.
...
If you need to use the Analytics API on the server side (provided your analytics provider is server side compatible), you can use the `analytics` object from the `@kit/analytics/server` package.
