...
NB: if you're running a version prior to 2.12.0, you must use the older authentication keys:
...
This key is available starting from Makerkit 2.12.0. If you're using a prior version, make sure to use `NEXT_PUBLIC_SUPABASE_ANON_KEY` instead.
...
This key is available starting from Makerkit 2.12.0. If you're using a prior version, make sure to use `SUPABASE_SERVICE_ROLE_KEY` instead.
